...
# -- coding: utf-8 --
# -------------------------------
# @功能：富贵论坛签到+固定范围活动评论（250-1048）
# @核心：ID区间遍历+时间戳去重+评论数量随机控制
# 在259行修改范围 print(f"✅ 生成可评论活动：{len(posts)}个（ID范围250-1048）")
# -------------------------------
...
class FGLTActivityTask:
    ...
    def _get_activity_posts(self):
        ...
        print(f"✅ 生成可评论活动：{len(posts)}个（ID范围250-1048）")
        return posts
    ...
    def do_activity_comments(self, session, username):
        ...
        if not posts:
            self.results.append("📊 评论：250-1048范围已无符合条件的活动（24小时内均已评论）")
            return
